...
from suzukabot import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, TIGER_USERS, GBAN_LOGS, LOGGER
from suzukabot.modules.helper_funcs.handlers import CMD_STARTERS
from suzukabot.modules.helper_funcs.misc import is_module_loaded, send_to_list
from suzukabot.modules.helper_funcs.chat_status import is_user_admin
from suzukabot.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
from suzukabot.modules.helper_funcs.string_handling import markdown_parser
from suzukabot.modules.disable import DisableAbleCommandHandler

import suzukabot.modules.sql.feds_sql as sql

from suzukabot.modules.connection import connected
from suzukabot.modules.helper_funcs.alternate import send_message
...
def new_fed(bot: Bot, update: Update):
	...
	if not fednam == '':
		...
		if not x:
			update.effective_message.reply_text("Can't federate! Please contact @suzukabotsupport if the problem persist.")
			return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
	...
	if len(args) >= 1:
		...
		if not x:
			message.reply_text("Failed to join federation! Please contact @suzukabotsupport should this problem persist!")
			return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
	...
	if fban:
		fed_name = info['fname']
		#https://t.me/suzukabotsupport/41606 // https://t.me/suzukabotsupport/41619
		...
		if not x:
			message.reply_text("Failed to ban from the federation! If this problem continues, contact @suzukabotsupport.")
			return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
	...
	if len(args) >= 1:
		...
		if not x:
			update.effective_message.reply_text("Whoa! There was an error while setting federation rules! If you wondered why please ask it in @suzukabotsupport !")
			return
